...
#---------------------------------------------#
# Variant counts and start sites
taiaroa_var_counts_SHORT_READ <- read_delim("short_reads/output_dataframes/taiaroa_cannonical_vs_variant_counts_SHORTREAD.tsv", delim="\t")
kim_var_counts_SHORT_READ <- read_delim("short_reads/output_dataframes/kim_cannonical_vs_variant_counts_SHORTREAD.tsv", delim="\t")
davidson_var_counts_SHORT_READ <- read_delim("short_reads/output_dataframes/davidson_cannonical_vs_variant_counts_SHORTREAD.tsv", delim="\t")

taiaroa_var_startsites_SHORT_READ <- read_delim("short_reads/output_dataframes/taiaroa_cannonical_vs_variant_startsites_SHORTREAD.tsv", delim="\t")
kim_var_startsites_SHORT_READ <- read_delim("short_reads/output_dataframes/kim_cannonical_vs_variant_startsites_SHORTREAD.tsv", delim="\t")
davidson_var_startsites_SHORT_READ <- read_delim("short_reads/output_dataframes/davidson_cannonical_vs_variant_startsites_SHORTREAD.tsv", delim="\t")
```
...
# REPEATING FOR THE SHORT-READ CORRECTED DATA...
#
#
# Plot variant counts
```{r}
...
# Define order
variant_cannnonical_listed_in_order <- c("ORF1a", "ORF1a_variant", "ORF1ab", "ORF1ab_variant", "S", "S_variant", "ORF3a", "ORF3a_variant", "E", "E_variant", "M", "M_variant", "ORF6", "ORF6_variant", "ORF7a", "ORF7a_variant", "ORF7b", "ORF7b_variant", "ORF8", "ORF8_variant", "N", "N_variant", "ORF10", "ORF10_variant")
...
# Need to get order for the ORFs that are present in davidson...
# This is because not all samples will have all variants and such.
variant_cannnonical_listed_in_order_taiaroa_SHORT_READ <- variant_cannnonical_listed_in_order[variant_cannnonical_listed_in_order %in% taiaroa_var_counts_SHORT_READ$ORF]

# Now, actually get the sample order for plotting
# Because of the coord flip in the plot, need to reverse the order.
taiaroa_order_SHORT_READ <- match(taiaroa_var_counts_SHORT_READ$ORF, rev(variant_cannnonical_listed_in_order_taiaroa_SHORT_READ))

# Add a column that details if the ORF is cannonical or variant
taiaroa_var_counts_SHORT_READ <- taiaroa_var_counts_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",ORF), "Variant", "Cannonical"))

# Plotting
taiaroa_var_counts_plt_SHORT_READ <- ggplot(data=taiaroa_var_counts_SHORT_READ, aes(x=reorder(ORF, taiaroa_order_SHORT_READ), y=Counts)) +
  geom_col(aes(fill=orf_type)) +
  coord_flip() +
  theme_minimal() +
  scale_fill_manual(values = c("#2471A3", "#A93226")) +
  ...
  scale_y_continuous(labels = comma) +
  scale_x_discrete(limits=rev(variant_cannnonical_listed_in_order))
taiaroa_var_counts_plt_SHORT_READ
...
# Need to get order for the ORFs that are present in davidson...
# This is because not all samples will have all variants and such.
variant_cannnonical_listed_in_order_kim_SHORT_READ <- variant_cannnonical_listed_in_order[variant_cannnonical_listed_in_order %in% kim_var_counts_SHORT_READ$ORF]

# Now, actually get the sample order for plotting
# Because of the coord flip in the plot, need to reverse the order.
kim_order_SHORT_READ <- match(kim_var_counts_SHORT_READ$ORF, rev(variant_cannnonical_listed_in_order_kim_SHORT_READ))

# Add a column that details if the ORF is cannonical or variant
kim_var_counts_SHORT_READ <- kim_var_counts_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",ORF), "Variant", "Cannonical"))

# Plotting
kim_var_counts_plt_SHORT_READ <- ggplot(data=kim_var_counts_SHORT_READ, aes(x=reorder(ORF, kim_order_SHORT_READ), y=Counts)) +
  geom_col(aes(fill=orf_type)) +
  coord_flip() +
  theme_minimal() +
  scale_fill_manual(values = c("#2471A3", "#A93226")) +
...
# Need to get order for the ORFs that are present in davidson...
# This is because not all samples will have all variants and such.
variant_cannnonical_listed_in_order_davidson_SHORT_READ <- variant_cannnonical_listed_in_order[variant_cannnonical_listed_in_order %in% davidson_var_counts_SHORT_READ$ORF]

# Now, actually get the sample order for plotting
# Because of the coord flip in the plot, need to reverse the order.
davidson_order_SHORT_READ <- match(davidson_var_counts_SHORT_READ$ORF, rev(variant_cannnonical_listed_in_order_davidson_SHORT_READ))

# Add a column that details if the ORF is cannonical or variant
davidson_var_counts_SHORT_READ <- davidson_var_counts_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",ORF), "Variant", "Cannonical"))

# Plotting
davidson_var_counts_plt_SHORT_READ <- ggplot(data=davidson_var_counts_SHORT_READ, aes(x=reorder(ORF, davidson_order_SHORT_READ), y=Counts)) +
  geom_col(aes(fill=orf_type)) +
  coord_flip() +
  theme_minimal() +
  scale_fill_manual(values = c("#2471A3", "#A93226")) +
  ...
  scale_y_continuous(labels = comma) +
  scale_x_discrete(limits=rev(variant_cannnonical_listed_in_order))
davidson_var_counts_plt_SHORT_READ
...
# Need to regenerate order for every single line (the many transcripts)
taiaroa_order_startsites_SHORT_READ <- match(taiaroa_var_startsites_SHORT_READ$subject, rev(variant_cannnonical_listed_in_order_taiaroa_SHORT_READ))

# Add information on variant or not
taiaroa_var_startsites_SHORT_READ <- taiaroa_var_startsites_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",subject), "Variant", "Cannonical"))

taiaroa_var_startsites_plt_SHORT_READ <- ggplot(data=taiaroa_var_startsites_SHORT_READ, aes(x=reorder(subject, taiaroa_order_startsites_SHORT_READ), y=start)) + 
  geom_boxplot(outlier.shape = NA, aes(fill=orf_type)) +
  coord_flip() +
  scale_y_continuous(labels = comma, limits = c(1, 5000)) +
  theme_minimal() +
  ...
  xlab("ORF") + ylab("Start Site on Transcript") +
  scale_x_discrete(limits=rev(variant_cannnonical_listed_in_order))
taiaroa_var_startsites_plt_SHORT_READ

# Kim
#--------------------#

# Need to regenerate order for every single line (the many transcripts)
kim_order_startsites_SHORT_READ <- match(kim_var_startsites_SHORT_READ$subject, rev(variant_cannnonical_listed_in_order_kim_SHORT_READ))

# Add information on variant or not
kim_var_startsites_SHORT_READ <- kim_var_startsites_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",subject), "Variant", "Cannonical"))

kim_var_startsites_plt_SHORT_READ <- ggplot(data=kim_var_startsites_SHORT_READ, aes(x=reorder(subject, kim_order_startsites_SHORT_READ), y=start)) + 
  geom_boxplot(outlier.shape = NA, aes(fill=orf_type)) +
  coord_flip() +
  scale_y_continuous(labels = comma, limits = c(1, 5000)) +
  theme_minimal() +
  ...
  xlab("ORF") + ylab("Start Site on Transcript") +
  scale_x_discrete(limits=rev(variant_cannnonical_listed_in_order))
kim_var_startsites_plt_SHORT_READ

# Davidson
#--------------------#

# Need to regenerate order for every single line (the many transcripts)
davidson_order_startsites_SHORT_READ <- match(davidson_var_startsites_SHORT_READ$subject, rev(variant_cannnonical_listed_in_order_davidson_SHORT_READ))

# Add information on variant or not
davidson_var_startsites_SHORT_READ <- davidson_var_startsites_SHORT_READ %>%
  mutate(orf_type = ifelse(grepl("variant",subject), "Variant", "Cannonical"))

davidson_var_startsites_plt_SHORT_READ <- ggplot(data=davidson_var_startsites_SHORT_READ, aes(x=reorder(subject, davidson_order_startsites_SHORT_READ), y=start)) + 
  geom_boxplot(outlier.shape = NA, aes(fill=orf_type)) +
  coord_flip() +
  scale_y_continuous(labels = comma, limits = c(1, 5000)) +
  theme_minimal() +
  ...
  xlab("ORF") + ylab("Start Site on Transcript") +
  scale_x_discrete(limits=rev(variant_cannnonical_listed_in_order))
davidson_var_startsites_plt_SHORT_READ
...
counts_and_startsites_plt_SHORT_READ <- (taiaroa_var_counts_plt_SHORT_READ + taiaroa_var_startsites_plt_SHORT_READ +
   theme(axis.title.y=element_blank(),
                              axis.text.y=element_blank(),
                              plot.title = element_blank())) /
  (kim_var_counts_plt_SHORT_READ + kim_var_startsites_plt_SHORT_READ +
     theme(axis.title.y=element_blank(),
                              axis.text.y=element_blank(),
                              plot.title = element_blank())) /
  (davidson_var_counts_plt_SHORT_READ + davidson_var_startsites_plt_SHORT_READ +
     theme(axis.title.y=element_blank(),
                              axis.text.y=element_blank(),
                              plot.title = element_blank()))


ggsave("figure2_ORFS_SHORT_READ.pdf", counts_and_startsites_plt_SHORT_READ, width = 20, height=20, units="cm")
ggsave("figure2_ORFS_SHORT_READ.svg", counts_and_startsites_plt_SHORT_READ, width = 20, height=20, units="cm")
```
