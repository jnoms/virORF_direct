...
# 5' Plotting Z-score distribution
```{r}

# Group by a span (e.g. 100bp), where the new span_percent is the sum of the percentages of non-canonical
# junctions at each position in that span.
merged_span_5 <- merged_5percentages %>%
  group_by(span = cut(five, breaks=seq(1,30000, by=100)), name) %>%
  summarize(span_percent=sum(percentage))

# Add stats
merged_span_5 <- merged_span_5 %>%
  group_by(span) %>%
  mutate(mean = mean(span_percent),
         sd = sd(span_percent),
         z = (span_percent - mean)/sd)
  

# Z-score distribution
span5_zscore_dist_plt <- merged_span_5 %>%
  ggplot() +
  geom_histogram(aes(x=z, fill=name),
                 binwidth = 0.1) + 
  facet_grid(rows=~name) +
  scale_fill_manual(values=c("#A93226", "#fca311", "#0096c7", "#7AC74F", "#053C5E")) +
  theme_minimal() +
  theme(legend.position="none") +
  ylab("Number of 100nt Spans") +
  theme(panel.spacing = unit(1, "lines"))
span5_zscore_dist_plt

ggsave("figures/dataset-specific-junctions/span5_zscore_dist_plt.pdf", span5_zscore_dist_plt, width=15, height=5, units="cm")
ggsave("figures/dataset-specific-junctions/span5_zscore_dist_plt.svg", span5_zscore_dist_plt, width=15, height=5, units="cm")
saveRDS(span5_zscore_dist_plt, "rds/dataset_specific_junctions/span5_zscore_dist_plt.rds")

```



# 3' Plotting Z-score distribution
```{r}
# Group by a span (e.g. 100bp), where the new span_percent is the sum of the percentages of non-canonical
# junctions at each position in that span.
merged_span_3 <- merged_3percentages %>%
  group_by(span = cut(three, breaks=seq(1,30000, by=100)), name) %>%
  summarize(span_percent=sum(percentage))

# Add stats
merged_span_3 <- merged_span_3 %>%
  group_by(span) %>%
  mutate(mean = mean(span_percent),
         sd = sd(span_percent),
         z = (span_percent - mean)/sd)
...
# Z score distribution
span3_zscore_dist_plt <- merged_span_3 %>%
  ggplot() +
  geom_histogram(aes(x=z, fill=name),
                 binwidth = 0.1) + 
  facet_grid(rows=~name) +
  ...
  ylab("Number of 100nt Spans") +
  theme(panel.spacing = unit(1, "lines"))
span3_zscore_dist_plt

ggsave("figures/dataset-specific-junctions/span3_zscore_dist_plt.pdf", span3_zscore_dist_plt, width=15, height=5, units="cm")
ggsave("figures/dataset-specific-junctions/span3_zscore_dist_plt.svg", span3_zscore_dist_plt, width=15, height=5, units="cm")
saveRDS(span3_zscore_dist_plt, "rds/dataset_specific_junctions/span3_zscore_dist_plt.rds")

```


# Making new tibble with replicates (5')
```{r}
taiaroa_5percentages <- to_5percentage(taiaroa, 'Taiaroa')
kim_5percentages <- to_5percentage(kim, 'Kim')
davidson_5percentages <- to_5percentage(davidson, "Davidson")
...
# Group by a span (e.g. 100bp), where the new span_percent is the sum of the percentages of non-canonical
# junctions at each position in that span.
merged_5percentages_replicates_span <- merged_5percentages_replicates %>%
  group_by(span = cut(five, breaks=seq(1,30000, by=100)), name) %>%
  summarize(span_percent=sum(percentage))

# Add stats
merged_5percentages_replicates_span <- merged_5percentages_replicates_span %>%
  group_by(span) %>%
  mutate(mean = mean(span_percent),
         sd = sd(span_percent),
         z = (span_percent - mean)/sd)
...
# Making new tibble with replicates (3')
```{r}
taiaroa_nc <- remove_canonical_3junctions(taiaroa)
kim_n <- remove_canonical_3junctions(kim)
davidson_n <- remove_canonical_3junctions(davidson)
...
kim_3percentages <- to_3percentage(kim_n, 'Kim')
davidson_3percentages <- to_3percentage(davidson_n, "Davidson")
finkel_24hpi_1_junc_3percentages <- to_3percentage(finkel_24hpi_1_junc_n, "Finkel_Rep1")
finkel_24hpi_2_junc_3percentages <- to_3percentage(finkel_24hpi_2_junc_n, "Finkel_Rep2")
SRR11517741_3percentages <-  to_3percentage(SRR11517741_n, "Blanco-Melo_Rep1")
SRR11517742_3percentages <-  to_3percentage(SRR11517742_n, "Blanco-Melo_Rep2")
SRR11517743_3percentages <-  to_3percentage(SRR11517742_n, "Blanco-Melo_Rep3")
...
# Group by a span (e.g. 100bp), where the new span_percent is the sum of the percentages of non-canonical
# junctions at each position in that span.
merged_3percentages_replicates_span <- merged_3percentages_replicates %>%
  group_by(span = cut(three, breaks=seq(1,30000, by=100)), name) %>%
  summarize(span_percent=sum(percentage))

# Add stats
merged_3percentages_replicates_span <- merged_3percentages_replicates_span %>%
  group_by(span) %>%
  mutate(mean = mean(span_percent),
         sd = sd(span_percent),
         z = (span_percent - mean)/sd)
...
# 5' Plotting junction distribution of 100bp spans
```{r}
# Visualize all ncJuncs in each span
merged_5percentages_replicates_span_plt <- merged_5percentages_replicates_span %>%
  ggplot() +
  geom_col(
    aes(x=span, y=span_percent, fill=name)
  ) +
  facet_grid(rows=vars(name)) +
  scale_fill_manual(values=c("#A93226", "#D24437", "#D96359", "#fca311", "#0096c7", "#00B8F5", "#7AC74F", "#053C5E")) +
  theme_minimal() +
  ...
  xlab("100nt Spans") + ggtitle("5' Non-canonical Junctions")
merged_5percentages_replicates_span_plt


merged_5percentages_replicates_span_plt

ggsave("figures/dataset-specific-junctions/merged_5percentages_replicates_span_plt.pdf", merged_5percentages_replicates_span_plt, width=15, height=15, units="cm")
ggsave("figures/dataset-specific-junctions/merged_5percentages_replicates_span_plt.svg", merged_5percentages_replicates_span_plt, width=15, height=15, units="cm")
saveRDS(merged_5percentages_replicates_span_plt, "rds/dataset_specific_junctions/merged_5percentages_replicates_span_plt.rds")
```



# 3' Plotting junction distribution of 100bp spans
```{r}
# Visualize all ncJuncs in each span
merged_3percentages_replicates_span_plt <- merged_3percentages_replicates_span %>%
  ggplot() +
  geom_col(
    aes(x=span, y=span_percent, fill=name)
  ) +
  ...
  theme(axis.text.x = element_blank(),
        ...
        strip.text.y.right = element_text(angle = 0, hjust=0)) + 
  ylab("Percentage of Non-canonical Junctions") +
  xlab("100nt Spans") + ggtitle("3' Non-canonical Junctions")
merged_3percentages_replicates_span_plt

ggsave("figures/dataset-specific-junctions/merged_3percentages_replicates_span_plt.pdf", merged_3percentages_replicates_span_plt, width=15, height=15, units="cm")
ggsave("figures/dataset-specific-junctions/merged_3percentages_replicates_span_plt.svg", merged_3percentages_replicates_span_plt, width=15, height=15, units="cm")
saveRDS(merged_3percentages_replicates_span_plt, "rds/dataset_specific_junctions/merged_3percentages_replicates_span_plt.rds")
```
...
# TMP - using this as a RULER to mark the ncJuncs from 2CD
```{r}

ruler5_tmp <- merged_5percentages_replicates_span %>% 
  mutate(span_percent=0) %>%
  mutate(span_percent=ifelse(span=="(2.73e+04,2.74e+04]", 1, 0)) %>%
  mutate(span_percent=ifelse(span=="(2.62e+04,2.63e+04]", 1, span_percent)) %>%
  ggplot() +
  geom_col(
    aes(x=span, y=span_percent, fill=name)
  ) +
...
ggsave("figures/dataset-specific-junctions/merged_5percentages_replicates_span_plt_RULER.svg",ruler5_tmp, width=15, height=15, units="cm")

merged_5percentages_replicates_span %>% pull(span) %>% table()



merged_3percentages_replicates_span %>% pull(span) %>% table()

ruler5_tmp <- merged_3percentages_replicates_span %>% 
  mutate(span_percent=0) %>%
  mutate(span_percent=ifelse(span=="(2.73e+04,2.74e+04]", 1, 0)) %>%
  mutate(span_percent=ifelse(span=="(2.91e+04,2.92e+04]", 1, span_percent)) %>%
  ggplot() +
  geom_col(
    aes(x=span, y=span_percent, fill=name)
  ) +
...
ggsave("figures/dataset-specific-junctions/merged_3percentages_replicates_span_plt_RULER.svg",ruler5_tmp, width=15, height=15, units="cm")
