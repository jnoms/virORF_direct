...
# Putting numbers on the difference in coverage in ORF1a - will compare before 1600 and after 1600 on ORF1A averages
```{r}

# ORF1A - from 266->13483


orf1a_region_coverage <- function(in_tbl, name, result_tbl=""){
  
  early_part <- in_tbl %>%
    filter(position>266, position<=1266) %>%
    pull(coverage) %>% mean()

  late_part <- in_tbl %>%
    filter(position>12438, position<13483) %>%
    pull(coverage) %>% mean()

  if (result_tbl == "") {
    result_tbl <- 
      tibble(name=name, before_1600=early_part, after_1600=late_part)
  } else {
    result_tbl <- result_tbl %>%
      add_row(name=name, before_1600=early_part, after_1600=late_part)
  }
...
orf1a_coverage_info <- orf1a_region_coverage(taiaroa_cov, "Taiaroa", "")
orf1a_coverage_info <- orf1a_region_coverage(kim_cov, "Kim", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(davidson_cov, "Davidson", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(finkel_24hpi_1_cov, "Finkel", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(SRR11517741_cov, "Blanco-Melo", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(emanual_total_24hpi_1_cov, "Emanual", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(ferret_cov, "Ferret", orf1a_coverage_info)
orf1a_coverage_info <- orf1a_region_coverage(suzuki_bronchial_5dpi_1_cov, "Suziki", orf1a_coverage_info)


orf1a_coverage_info <- orf1a_coverage_info %>%
  mutate(fold_dif = before_1600/after_1600)
orf1a_coverage_info


mean(orf1a_coverage_info$fold_dif)
sd(orf1a_coverage_info$fold_dif)
median(orf1a_coverage_info$fold_dif)
